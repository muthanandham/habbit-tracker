...
export default function Journal() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
        ...
        <div className="card-elevated p-6 space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
          ...
          <div className="flex items-center gap-6 py-2 border-y border-white/5">
            <div className="flex items-center gap-3">
              <span className="text-xs font-mono text-text-secondary uppercase tracking-widest">Mood</span>
              <input 
                type="range" min="1" max="10" 
                value={newMood}
                onChange={(e) => setNewMood(parseInt(e.target.value))}
                className="w-24 h-1 bg-bg-surface rounded-full appearance-none cursor-pointer accent-accent-journal"
              />
              <span className="text-sm font-mono text-accent-journal">{newMood}</span>
            </div>
            <div className="flex-1">
              <input
                ...
                className="w-full bg-transparent text-sm border-none focus:ring-0 placeholder:text-text-secondary/30"
              />
            </div>
          </div>
      ...
      {/* Entries List */}
      <div className="space-y-12 relative before:absolute before:left-[17px] before:top-2 before:bottom-0 before:w-px before:bg-white/5">
        {entries.map((entry) => (
          <div key={entry._id} className="relative pl-12 group">
            {/* Timeline Dot */}
            <div className="absolute left-0 top-1.5 w-9 h-9 rounded-full bg-bg-app border border-white/10 flex items-center justify-center z-10 group-hover:border-accent-journal transition-colors">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-heading font-semibold text-text-primary group-hover:text-accent-journal transition-colors">
                    {entry.title || 'Untitled Entry'}
                  </h3>
                  <p className="text-text-secondary text-xs font-mono uppercase tracking-widest mt-1">
                    {new Date(entry.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                  </p>
                </div>
                <button 
                  onClick={() => deleteEntry(entry._id)}
                  className="p-2 opacity-0 group-hover:opacity-100 hover:bg-white/5 rounded-lg transition-all text-text-secondary hover:text-red-400"
                >
              ...
              {entry.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 pt-2">
                  {entry.tags.map(tag => (
                    <span key={tag} className="px-2 py-0.5 rounded bg-white/5 text-[10px] text-text-secondary uppercase tracking-widest border border-white/5">
                      #{tag}
                    </span>
                  ))}
                </div>
